...
- Exercise: there's two identical versions of the exercise, participants can pick either one. Often participants ask further explanations about our scripts, in particular as there is a lot of bash programming. You can point them to the [Unix materials](https://cambiotraining.github.io/unix-shell/materials/07-variables.html) if they are not familiar with bash variables.
...
- Slides 58-71: these go through Filezilla, scp and rsync.
  - Based on feedback from previous courses, there are now slides schematically illustrating how `rsync`/`scp` work, as the syntax can be confusing for those.
